"""Compute ResNet-50 logits on real ImageNet val (HF parquet shards) and cache as .npy.

Loads:
  $SCORC_DATA_DIR/imagenet_data/data/validation-*.parquet (14 shards, ~50k rows total)
Outputs:
  $SCORC_DATA_DIR/imagenet_data/val_logits.npy   # (50000, 1000) float32
  $SCORC_DATA_DIR/imagenet_data/val_labels.npy   # (50000,) int64

Model: torchvision.models.resnet50(weights=ResNet50_Weights.IMAGENET1K_V2)
       — the standard ImageNet val benchmark (~80% top-1 acc).
"""
...
def main():
    ...
    print(f"[logits] top-1 accuracy on val: {acc:.4f} (expected ~0.80 for ResNet-50 V2)")

    logits_path = out_dir / "val_logits.npy"
